---
class: PoeticWork
label: Quanti son poi, che divenuti amanti
description:
identifier:
described_by_source:
image:
aliases:
instance_of:
  - "[[poem]]"
contributor:
  - role: "[[author]]"
    value: "[[Veronica Gambara]]"
part_of:
  - value: "[[Rime di diversi eccellenti autori bresciani]]"
    section: Veronica Gambara.
    pages: 10
local_asset_path: ./assets/poems/quanti_son_poi,_che_divenuti_amanti
based_on:
derivative_work:
subject:
poetic_type: "[[ottava rima]]"
metre:
  - "[[hendecasyllable]]"
depicts:
  - "[[lover]]"
  - "[[entity/weeping|weeping]]"
text: |-
  Quanti son, poi, che divenuti amanti
  Di due begli occhi, e d'un leggiadro viso,
  Si pascon sol di dolorosi pianti,
  Da se stessi tenendo il cor diviso,
  Né gioia né piacer sono bastanti
  Trar lor del petto, se non finto riso,
  E se lieti tal'hor si mostran fuori,
  Hanno per un piacer, mille dolori.
language: "[[Italian]]"
---
